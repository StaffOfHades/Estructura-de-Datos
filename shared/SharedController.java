...
public class SharedController implements Initializable {
    ...
    //region Properties
    private final static String CLASS = "el arbol";
     ...
     * If the binary tree is not empty, it asks the user for an int to be found & erased in
     * the binary tree
     * . If no match is found, the user receives an alert.
     * @param actionEvent ActionEvent sent by the caller
     */
    @FXML
    public void handleEraseAction(ActionEvent actionEvent) {
        // Check to see if binary tree
        // is not empty
        if ( !checkArrayEmpty() ) {
            ...
            // If a number was returned, start looking in the binary tree
     ...
     * If the binary tree
     * is not full, it asks the user for an int to be added into the binary tree
     * .
     * @param actionEvent  ActionEvent sent by the caller
     */
    @FXML
    public void handleInsertAction(ActionEvent actionEvent) throws IOException {
        // Check to see if the binary tree
        // is not full
        if ( !checkArrayFull() ) {
            // Ask the user for a number, parses it, and returns it.
            // If the operation failed, the user is alerted of the reason why,
            ...
            // If a number was returned, add it to the binary tree
            // ,
            // & update the window
            if (num != null) {
                tree.add(num);
     ...
     * If the binary tree
     * is not full, fileChooser shows an open dialog.
     ...
     * space in the binary tree
     * . Only numbers are added to the binary tree
     * ,
    ...
    public void handleOpenAction(ActionEvent actionEvent) throws IOException {
        ...
        if ( !checkArrayFull() ) {
            ...
            */
            ...
            // and there is space to add in the binary tree
            // .
            while ( (line = reader.readLine() ) != null && tree.hasSpace() ) {
                // Try to parse the line into an acceptable number.
                // Otherwise, ignore the line
                try {
                    num = Integer.parseInt(line);

                    // Add the number to binary tree
     ...
     * If the binary tree
     * is not empty, search the binary tree
     * for a match.
     * @param actionEvent ActionEvent sent by the caller
     */
    @FXML
    public void handleFindAction(ActionEvent actionEvent) {
        ...
        if (!checkArrayEmpty()) {
            ...
            // is found in the binary tree
            // .
            if (num != null) {
                // Create the body text according to whether a match is found
                String text = "El numero " + num + (tree.hasElement(num) ? " " : " no ") + "existe en " + CLASS;
    ...
    public void handleShowAction(ActionEvent actionEvent) {
        ...
        // Otherwise, if show data is true, just add all the numbers in the binary tree
        // to the window
        if ( actionEvent.getSource() == visibility) {
            // If show data is false, add the numbers in the binary tree
            // to the window,
            // & set the menu item text to hide.
            // Otherwise, set the menu item to show
            if (!showData) {
                text_area.appendText( tree.toString() );
    ...
    private boolean checkArrayEmpty() {
        ...
        dialog.setContentText("No hay ningun dato en " + CLASS);
        dialog.showAndWait();
    ...
    /**
     * checkArrayFull checks if the binary tree
     * is full, and if so,
     * alerts the user of this fact.
     * @return if binary tree
     * is full
